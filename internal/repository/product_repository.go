...
func (pr *productRepository) CreateNewProduct(ctx context.Context, product *entity.Product) error {
	_, err := pr.db.ExecContext(
		ctx,
		`INSERT INTO products (id, name, description, price, image_file_name, created_at, created_by, updated_at, updated_by, deleted_at, deleted_by, is_deleted)
...
func (pr *productRepository) DeleteProduct(ctx context.Context, id string, deletedAt time.Time, deletedBy string) error {
	_, err := pr.db.ExecContext(
		ctx,
		`UPDATE products SET deleted_at=$1, deleted_by=$2, is_deleted = true WHERE id=$3`,
		deletedAt,
		deletedBy,
		id,
	)
...
func (pr *productRepository) GetProductsPagination(ctx context.Context, pagination *common.PaginationRequest) ([]*entity.Product, *common.PaginationResponse, error) {

	row := pr.db.QueryRowContext(ctx, `SELECT COUNT(*) FROM products WHERE is_DELETED = false`)
	if row.Err() != nil {
	...
	rows, err := pr.db.QueryContext(
		ctx,
		`SELECT id, name, description, price, image_file_name FROM products WHERE is_deleted = false ORDER BY created_at DESC LIMIT $1 OFFSET $2`,
...
func (pr *productRepository) GetProductsPaginationAdmin(ctx context.Context, pagination *common.PaginationRequest) ([]*entity.Product, *common.PaginationResponse, error) {
	...
	baseQuery := fmt.Sprintf(`SELECT id, name, description, price, image_file_name FROM products WHERE is_deleted = false %s LIMIT $1 OFFSET $2`, orderQuery)
	rows, err := pr.db.QueryContext(
		ctx,
		baseQuery,
		pagination.ItemPerPage,
