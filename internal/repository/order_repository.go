...
func (or *orderRepository) CreateOrder(ctx context.Context, order *entity.Order) error {
	_, err := or.db.ExecContext(
		ctx,
		`INSERT INTO "order" (id, number, user_id, order_status_code, 
		user_full_name, address, phone_number, notes, total, 
		expired_at, created_at, created_by, updated_at, 
		updated_by, deleted_at, deleted_by, is_deleted, xendit_invoice_id, xendit_invoice_url) 
...
func (or *orderRepository) CreateOrderItem(ctx context.Context, orderItem *entity.OrderItem) error {
	_, err := or.db.ExecContext(
		...
		(id, product_id, product_name, product_image_file_name, product_price, 
		quantity, order_id, created_at, created_by, updated_at, 
...
func (or *orderRepository) UpdateOrder(ctx context.Context, order *entity.Order) error {
	_, err := or.db.ExecContext(
		ctx,
		`UPDATE "order" SET updated_at = $1, updated_by = $2, xendit_paid_at = $3, 
		xendit_payment_channel = $4, xendit_payment_method = 
		$5, order_status_code = $6 WHERE id = $7`,
		order.UpdatedAt,
		order.UpdatedBy,
		order.XenditPaidAt,
