...
func (cr *cartRepository) GetListCart(ctx context.Context, userId string) ([]*entity.UserCart, error) {
	rows, err := cr.db.QueryContext(
		ctx,
		`SELECT uc.id, uc.product_id, uc.user_id, uc.quantity, uc.created_at, uc.created_by, uc.updated_at, uc.updated_by, 
		p.id, p.name, p.image_file_name, p.price 
		FROM user_cart uc JOIN products p ON uc.product_id = p.id 
		WHERE uc.user_id = $1 AND p.is_deleted = false`,
		userId,
	)
